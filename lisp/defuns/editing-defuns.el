...
If there's no region, the current line will be duplicated."
  ...
  (if (region-active-p)
      (let ((beg (region-beginning))
            (end (region-end)))
        (duplicate-region arg beg end)
        (one-shot-keybinding "d" (lam (duplicate-region 1 beg end))))
    (duplicate-current-line arg)
    (one-shot-keybinding "d" 'duplicate-current-line)))

(defun comment-line-dwim (&optional arg)
  "If no region is selected, toggle comment of current line. Otherwise, call comment-dwim."
  (interactive "*P")
  ...
  (let ((path nil))
    (save-excursion
      (save-restriction
        ...
        (let ((path-string (format "/%s" (mapconcat 'identity path "/"))))
